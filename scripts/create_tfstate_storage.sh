...
export TF_VAR_tfstate_storageaccount_key=$ACCOUNT_KEY

# Store name of storageaccount as environment variable
export TF_VAR_tfstate_storageaccount_name=$STORAGE_ACCOUNT_NAME

# create dynamic init arguments
export TF_CLI_ARGS_init="-backend-config='storage_account_name=$STORAGE_ACCOUNT_NAME' -backend-config='resource_group_name=$RESOURCE_GROUP_NAME' -backend-config='access_key=$ACCOUNT_KEY' -backend-config='container_name=jiratfstate' -backend-config='key=terraform.tfstate'"